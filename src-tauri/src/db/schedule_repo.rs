use crate::{errors::AppError, models::schedule::{ScheduleEntry, ScheduleEntryView, ScheduleView}};
use sqlx::SqlitePool;
use uuid::Uuid;

pub async fn get_by_event(pool: &SqlitePool, event_id: &str) -> Result<ScheduleView, AppError> {
    let event = sqlx::query!(
        "SELECT id, name, event_date FROM events WHERE id = ?", event_id)
        .fetch_optional(pool).await.map_err(AppError::from)?
        .ok_or_else(|| AppError::NotFound(format!("Event '{}' not found", event_id)))?;

    let entries = sqlx::query!(
        r#"SELECT se.id as entry_id, se.squad_id, s.name as squad_name,
                  se.member_id, m.name as member_name
