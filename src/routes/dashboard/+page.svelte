    ...
    <div class="grid grid-cols-3 gap-4 mb-8">
      ...
      <div class="card flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-xs font-semibold text-slate-500 mt-0.5">Membros</p>
          <p class="text-xs text-slate-500">{activeMembers()} ativos</p>
        </div>
      </div>

      <!-- Times -->
      <div class="card flex items-center gap-4">
        <div class="w-11 h-11 rounded-lg flex items-center justify-center shrink-0" style="background:#ede9fe">
        ...
        <div>
          ...
          <p class="text-xs font-semibold text-slate-500 mt-0.5">Times</p>
          <p class="text-xs text-slate-500">{squadsWithMembers} c/ membros</p>
        </div>
      </div>

      <!-- Eventos -->
      <div class="card flex items-center gap-4">
        <div class="w-11 h-11 rounded-lg flex items-center justify-center shrink-0" style="background:#fef9c3">
        ...
        <div>
          ...
          <p class="text-xs font-semibold text-slate-500 mt-0.5">Eventos</p>
          <p class="text-xs text-slate-500">{eventsThisMonth} este mês</p>
        </div>
      </div>
    </div>

    <!-- Próximo Evento -->
    <h2 class="text-base font-bold mb-3 text-slate-500 uppercase tracking-wide">Próximo Evento</h2>
    {#if nextEvent}
      <div class="card mb-8">
        <div class="flex items-start justify-between gap-4">
          <div>
            <p class="text-lg font-bold">{nextEvent.name}</p>
            <p class="text-sm text-slate-500 mt-0.5">{nextEvent.event_date ? formatDateLong(nextEvent.event_date) : ''}</p>
            <span class="badge badge-blue mt-2">{nextEvent.event_type}</span>
          </div>
          ...
          {:else}
            <a href="/events" class="text-sm text-amber-700 font-semibold whitespace-nowrap no-underline">⚠ Sem times — Configurar →</a>
          {/if}
        </div>
      </div>
    {:else}
      <div class="card mb-8 text-center p-8 text-slate-500">
        <p class="mb-2">Nenhum evento próximo.</p>
        <a href="/events" class="btn btn-primary inline-block">+ Criar Evento</a>
      </div>
    {/if}

    <!-- Alertas -->
    <h2 class="text-base font-bold mb-3 text-slate-500 uppercase tracking-wide">Alertas</h2>
    {#if eventsWithoutSquads === 0 && membersWithoutSquad === 0}
      <div class="card flex items-center gap-3 text-green-600">
